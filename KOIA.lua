...
os.execute('chmod +x KOIA;chmod +x KOIA;./KOIA')
end
...
KOIA = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..KOIA)
LuaTele = luatele.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=KOIA,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(KOIA.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(KOIA) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(KOIA.."chfalse")
if chh then
...
if msg.sender.user_id == tonumber(KOIA) then
return false
end
if Redis:get(KOIA.."chsource") then
chsource = Redis:get(KOIA.."chsource")
else
chsource = "CR_T2"
end
...
local Info_User = Redis:get(KOIA.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(KOIA..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'⌯ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(KOIA..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'⌯ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "الغاء الامر" then
Redis:del(KOIA..msg.sender.user_id.."bottoken")
Redis:del(KOIA..msg.sender.user_id.."botuser")
Redis:del(KOIA..msg.sender.user_id.."make:bot")
...
Redis:set(KOIA..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"⌯ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(KOIA..msg.sender.user_id.."stop:bot") then
Redis:del(KOIA..msg.sender.user_id.."stop:bot")
Redis:del(KOIA.."screen:on")
Redis:del(KOIA.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(KOIA..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"⌯ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(KOIA.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(KOIA.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(KOIA.."chfalse")
Redis:set(KOIA.."chfalse",ch)
Redis:del(KOIA.."ch:admin")
Redis:set(KOIA.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'⌯ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "تفعيل الاشتراك الاجباري" then
Redis:set(KOIA.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(KOIA..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(KOIA..msg.sender.user_id.."bottoken")
local botuser = Redis:get(KOIA..msg.sender.user_id.."botuser")
...
Redis:sadd(KOIA.."userbots",botuser)
Redis:del(KOIA..msg.sender.user_id.."bottoken")
Redis:del(KOIA..msg.sender.user_id.."botuser")
Redis:del(KOIA..msg.sender.user_id.."make:bot")
...
if text and Redis:get(KOIA..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(KOIA.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n⌯ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(KOIA..msg.sender.user_id.."botuser",botuser)
Redis:set(KOIA..msg.sender.user_id.."bottoken",text)
Redis:set(KOIA..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(KOIA..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n⌯ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(KOIA.."all:texting") then
if text == "الغاء" or text == 'الغاء الامر' then   
Redis:del(KOIA.."all:texting")
send(msg.chat_id,msg.id, "\n⌯ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(KOIA.."3z:gp",text)
Redis:del(KOIA.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(KOIA.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(KOIA.."3z:gp")
Redis:del(KOIA.."all:texting")
send(msg.chat_id,msg.id,"⌯ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(KOIA.."all:texting:pv") then
if text == "الغاء" or text == 'الغاء الامر' then   
Redis:del(KOIA.."all:texting:pv")
send(msg.chat_id,msg.id, "\n⌯ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(KOIA.."eza3a:pv",text)
Redis:del(KOIA.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(KOIA.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(KOIA.."eza3a:pv")
Redis:del(KOIA.."all:texting:pv")
send(msg.chat_id,msg.id,"⌯ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "اذاعه عام للمجموعات" then
Redis:set(KOIA.."all:texting",true)
send(msg.chat_id,msg.id,"⌯ ارسل النص الان","html",true)
end
if text == "اذاعه عام للمشتركين" then
Redis:set(KOIA.."all:texting:pv",true)
send(msg.chat_id,msg.id,"⌯  ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "فحص" then
Redis:del(KOIA.."All:pv:st")
Redis:del(KOIA.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(KOIA.."All:gp:st",tonumber(#list))
Redis:incrby(KOIA.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\nاحصائيات جميع البوتات المصنوعه \n⌯ عدد المجموعات '..Redis:get(KOIA.."All:gp:st")..' مجموعه \n⌯ عدد المشتركين '..Redis:get(KOIA.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
-------delete 
if text and Redis:get(KOIA..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == 'الغاء الامر' then   
Redis:del(KOIA..msg.sender.user_id.."make:bot")
...
Redis:srem(KOIA.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n⌯ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "حذف بوت" then
Redis:set(KOIA..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(KOIA..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(KOIA..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(KOIA..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == 'الغاء الامر' then   
Redis:del(KOIA..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n⌯ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(KOIA.."chsource",ch)
...
Redis:set(KOIA..msg.sender.user_id.."setchannel",true)
...
Redis:set(KOIA.."twsl",true)
send(msg.chat_id,msg.id,"⌯ تم تعطيل التواصل")
return false 
end
if text == "الاحصائيات" then
local list = Redis:smembers(KOIA.."total")
...
if tonumber(KOIA) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'⌯ عذرا هذا الملف غير متطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(KOIA..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(KOIA.."total")
local Get_Json = '{"BotId": '..KOIA..','  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(KOIA..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == 'الغاء الامر' then   
Redis:del(KOIA..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n⌯ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(KOIA.."total") 
...
if text == "اذاعه" then
Redis:set(KOIA..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"⌯  ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(KOIA..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == 'الغاء الامر' then   
Redis:del(KOIA..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(KOIA..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(KOIA.."ch:admin")
send(msg.chat_id,msg.id,'\n⌯ عليك الاشتراك في قناة البوت لاستخدام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(KOIA.."twsl") then
if msg.sender.user_id ~= tonumber(KOIA) then
if Redis:sismember(KOIA..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(KOIA.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(KOIA.."free:bot") then
return send(msg.chat_id,msg.id,"⌯ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(KOIA.."total",msg.sender.user_id) then
Redis:sadd(KOIA.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
---making user
if text and Redis:get(KOIA..msg.sender.user_id.."make:bot") then
if text == "الغاء" then
Redis:del(KOIA..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(KOIA..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(KOIA.."userbots",botuser)
Redis:del(KOIA..msg.sender.user_id.."make:bot")
...
if text == "صنع بوت" then
if Redis:get(KOIA..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"⌯  عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(KOIA..msg.sender.user_id.."make:bot",true)
...
if Redis:get(KOIA..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(KOIA..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(KOIA.."userbots",botuser)
Redis:del(KOIA..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n⌯ تم حذف البوت بنجاح","md",true)  
else
...
if tonumber(data.message.sender.user_id) == tonumber(KOIA) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == KOIA then
return false
end
